...
Let $P = \neg WP(False, \pi[i,n])$.\\
Let $Q =  WP(False, \pi[i+1,n])$.\\
The statement $\pi[i]$ is relevant iff $P \Rightarrow WP(Q,Havoc(x))$
\end{theorem}
Proof: Let us first define the following 2 lemmas: \\
\begin{lemma}
...
The statement is relevant if $\lbrace p \rbrace havoc(s) \lbrace \neg Q \rbrace$ is unsatisfiable and $havoc(S)$ is in the unsatisfiable core. \\
That means in  $\lbrace p \rbrace havoc(s) \lbrace Q \rbrace$ should not be in the unsatisfiable core. \\
or we can write: $$P \Rightarrow WP(Q,Havoc(S))$$
