...
An execution $\sigma$ of a trace $\pi$ of length $n$ is a sequence of states $s_0....s_n$ such that for all $0 \leq i \leq n$, $s_i,s_{i+1} \Rightarrow T[\pi[i]]$. We denote by $Execs(\pi)$ the set of all executions of $\pi$. The trace formula of $\pi$ is the formula:
...
If a program is not safe, an error can be witnessed along a trace. The error corresponds to a set of executions that violate the assertion associated with the last label of the trace. We call it correctness assertion as mentioned in the introduction section of this sub-section. \\
\textbf{Error \& Error Trace: }For an unsafe program $P=<st, \Phi>$ , with a trace $\pi = st_0^a;.....st_{n-2}^a;label\ l$, an error precondition $\Psi$, a state formula $\phi$, such that $\Phi(l)=\phi$, we say that there is an error in the program $P$ if the following conditions hold :\\
1) $\Psi \wedge TF(\pi)$ is satisfiable\\
2) $\Psi \wedge TF(\pi) \wedge \phi^{<n>} $ is unsatisfiable\\
In this case $\pi$ is called an error trace. Hence for an error trace $\pi$, no execution of the trace $\pi$ that starts in a state satisfying the \textbf{\textit{error pre-condition}} $\Psi$ ends in a state satisfying the postcondition $\phi$.
