...
In the Error invariants paper \cite{errorinvariants} .. \\
Explaining inconsistent code paper .. \\
...
Let $ST$ be a set containing all the statements in a program. $\pi \in ST$ is called an Error Trace iff there is an assertion condition $\phi$, which is violated if we execute $\pi$, starting from a state which belongs to a set of states called Error Precondition $EP$. We call the violation of $\pi$ an error.\\
...
One possible definition of relevancy could be that in an error trace $\pi$, an assignment statement $\pi[i]$ is relevant if after replacing that statement with havoc (non-deterministic input), then we can't be certain that the assertion condition $\phi$ will be violated any more.\\
For example:
\begin{lstlisting}
foo()
{
...
assume( y==3 )
\end{lstlisting}
If we replace $y:=3$ by $hacov(y)$, then $y$ can non-deterministically be assigned any value and we can't be certain any more that the assertion condition in the end $\phi = (y!=3)$ will be violated.
...
An error is a security error iff\\
We say that a program has a security bug, if input (user input/network input)
can trigger that some execution reaches the error.

A definition/criterion that Vadim and I discussed was the following.
An error is a security error iff 

1 there is some reachable location linput where the program reads input, and
2. there is some input that continuing from this location we definitely reach
the error, and
3. there is some input that continuing from this location we do not reach the
error.
\\
Note that condition (3.) states something like the input is relevant for the
error.

Are we able to extend our relevance analysis in a way that it solves Vadim's
problem?

My first guess was that problem is easy. Whenever we say that the user input is
relevant we know that we found a security bug. Unfortunately, my guess was
wrong. Attached is an example.

Our tool says (correctly) that copyfromuser() is relevant. However, the
program does not have a security bug (according to our definition). For each
result of copyfromuser() there is no guarantee that we reach the error since
the function f() for which no implementation is given may cause that remaining
statement sequence is blocking.

We may assume that we are able to distinguish functions that provide user
input (here copyfromuser() ) and functions whose behavior is not defined
(here f() ). (I think I could implement this relatively soon.)
