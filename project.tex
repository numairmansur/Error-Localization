...
\subsection{Non-Flow Sensitive Relevance criteria }
 ...
 \\
 Lets call $\phi$ a post-condition and $\phi = \neg WP(False, \pi[i+1,n])$.\\
 Lets call $\Psi$ a pre-condition and $\Psi = \neg WP(False, \pi[i,n])$.\\
 Where $n$ is the length of the error trace.\\
 \\
 A statement $\pi[i]$ in an error trace $\pi$ is relevant with respect to the non-flow sensitive criteria $(*)$ if the conjunction of the the three formulas is unsatisfiable and $\pi[i]$ is in the unsatisfiable core.\\
\\
\end{document}