...
The formulas $Preds(X \cup X')$ are called \textbf{\textit{transition formulas}}. We use transition formulas to represent the semantics of atomic statements in a trace, where the variables $X'$ denote the values of the variables from $X$ in the next state. We write $s,s' \Rightarrow T$ to denote that states $s$ and $s'$ satisfy the transition formula $T$.\\
...
If a program is not safe, an error can be witnessed along a trace. The error corresponds to a set of executions that violate the assertion associated with the last label of the trace. We call it the violated \textbf{\textit{correctness assertion}} as mentioned in the introduction section of this sub-section. \\
\textbf{Error \& Error Trace: }For an unsafe program $P=<st, \Phi>$ , with a trace $\pi = st_0^a;.....st_{n-2}^a;label\ l$, an error precondition $\Psi$, a state formula $\phi$, such that $\Phi(l)=\phi$, we say that there is an \textbf{\textit{error}} in the program $P$ if the following conditions hold :\\
...
\textbf{Error Pre-condition: } ...


\subsubsection{Definition}
We say that in a error trace $\pi$, an assignment statement $\pi[i]$ is relevant w.r.t non-flow sensitive relevancy criteria if after replacing that statement in the trace with a havoc statement to get a new trace $\pi'$ , there exists an execution $\sigma'$ in $Execs(\pi')$, such that $\Psi \wedge TF(\pi') \wedge \phi^{<n>} $ is satisfiable.
\\
For example:
\begin{lstlisting}
foo()
