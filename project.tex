...
2) Error Trace\\
If we see it from the angle of \textbf{Program level scope}, then we can say that the execution of the program will not terminate and the program is stuck in a non-terminating loop or something.\\
But ! if we see it from an angle of the \textbf{error trace}, then it could mean that now you are getting blocked in the error trace and an assume statement can now become a \textit{\textbf{blocking statement}}. Now how to formalize this ? an assume statement becoming a blocking statement.
