...
An assignment statement is relevant if the assigned value matters for the reachibility of the error. If we are reaching the error state for any possible value, then the assignment is irrelevant. or equivilalntly we can say that in an error trace $\pi$ an assignment statement in an error trace is relevant w.r.t non-flow sensitive relevancy criteria if after replacing that statement with a non-deterministic $havoc()$ to get a new trace $\pi'$, we can't guarantee anymore that we will reach the error state.
For example:
\begin{lstlisting}
foo()
{
...
Formally: \\
$\pi[i]$ is relevant if in a trace $\pi'$, where $\pi'[i]$ is a havoc statement in terms of the variable on the left side of $\pi[i]$ and $\exists$ some assume statement at position $j$ such that \\
$$post(\pi'[0,j-1], pre) \not \subseteq guard(\pi[j])$$ 
