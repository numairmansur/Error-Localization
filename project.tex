...
-If we start the execution of the error trace from a state satisfying the error-precondition $\Psi$, then we will always end up in an error state. \\
- If we start the execution of an error trace from a state other then the error-precondition, then we can get \textit{\textbf{stuck}}.\\
