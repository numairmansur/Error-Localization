...
We can give the formal description by making the following observations: \\
1) If we start the execution of the program from a state satisfying the error-precondition $\Psi$, then if we start the execution of the program from that state then we will always end up in an error state. \\
2) If we start the execution of an error trace from a state other then the error-precondition, then we can get \textit{\textbf{stuck}} during the execution of the error trace .\\
3) If we start the execution of an error trace from a state satisfying the error-precondition and we replace an assignment statement in the error trace with a havoc statement and we get stuck again after doing that,  then that statement is called a \textit{\textbf{relevant statement}}.\\
\textbf{What does it mean to get stuck?}\\
Getting stuck could mean that now we are getting blocked in the error trace and an assume statement can now become a \textit{\textbf{blocking statement}}. By blocking statement, we mean that we cannot get passed that statement because the assumption is not true any more. [[Because in an error trace there are only assignment statements or assume statements. Assignment statements can't become blocking, only assume can. Now how to formalize this ? An assume statement becoming a blocking statement ]]\\
