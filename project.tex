...
From lemma (2)
\begin{equation}
WP(guard, x:=t) \Rightarrow WP(guard, havoc(x))
\end{equation}
does not hold.\\
From the "duality of WP and SP" (lemma 1), if in the above expression, we consider the right hand side of the implication as $P$, $guard$ as $Q$ and $havoc(x)$ as $st$, then we can write the above implication as: \\
\begin{equation}
SP(WP(guard, x:=t), havoc(x)) \Rightarrow guard
\end{equation}
The above implication also does not hold as it is equivalent to (4). \\
That means that from our supposed trace which contains one assignment statment and one assume statement with error precondition $WP(guard,x:=t)$, if we get a new trace where the assignment $x:=t$ is replaced by $havoc(x)$, the strongest postcondition of the error precondition and $havoc(x)$ does not imply the guard of the assume statement. Which according to the definition of relevancy would mean that the statement $x:=t$ is relevant.\\
\\ Now we consider the case where the length of the trace is $n$ and the first statment of the trace is an assignment statement. We consider this assignment statement for now.\\
$"\Rightarrow"$\\
If the first staement $st$ is relevant, then the following implication does not hold.
\begin{equation} 
P \Rightarrow WP(Q,havoc(x)) 
\end{equation}
If the statment $st$ is relevant, then there exists some assume statement in the trace at position $j$, which is restrictive if we replace the assignment statement with a havoc statement.\\
We know that
\begin{equation} 
SP(P, havoc(x),\pi[1,j-1]) \Rightarrow guard(\pi[j]) 
\end{equation}
does not hold.
and consequently from lemma (1):
\begin{equation} 
P \Rightarrow WP(guard(\pi[j]); havoc(x),\pi[1,j-1])
\end{equation}
does not hold.\\
We also know that:
\begin{equation} 
...
and consequently from lemma (1):
\begin{equation} 
Q \Rightarrow WP(guard(\pi[j]); \pi[1,j-1])
\end{equation}
Expression (8) can be written as 
\begin{equation} 
P \Rightarrow WP( WP(guard(\pi[j]); \pi[1,j-1]); havoc(x))
\end{equation}
does not hold.\\
Substituting from (10) and we can write the above form expression as:
\begin{equation} 
P \Rightarrow WP(Q; havoc(x))
\end{equation}
does not hold.
...
\subsubsection*{Proof:}
Consider first a case where we have an error trace $\pi$ of length 2, containing one havoc statement and one assume statement. Let $guard(x)$ be the guard of the assume statement. We take into account the $havoc$ statement at position $i=0$.\\
$"\Rightarrow"$\\
The $havoc$ statement is relevant if the following implication does not hold.
$$P \Rightarrow WP(Q,havoc(x))$$
For the initial case $P$, will be $\neg WP(\neg guard, havoc(x))$, which is the error precondition of the trace and $Q$ will be $guard$.\\
Hence we have to show that the following implication does not hold.
\begin{equation}
\neg WP(\neg guard, havoc(x)) \Rightarrow WP(guard, havoc(x))
\end{equation}
It can be shown that the above implication does not hold, if \\
1) The left hand side of the implication is not false.\\
2) Right hand side of the implication is false.\\
...
Which is the left hand side of the implication in (13).
Since the right hand side of the implication in (13) is the error precondtion of a feasible error trace, it can not be empty or false. \\
Hence the implication in (13) does not hold.\\
$"\Leftarrow"$\\
The statement $havoc(x)$ is relevant if the following implication does not hold. 
$$P \Rightarrow WP(Q,havoc(x))$$
