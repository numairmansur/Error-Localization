...
A program automaton is a finite automaton. A state in a program automaton corresponds to a program location, and transitions between two states are labelled with program statements.
\\
\textbf{Error Trace:} Let $ST$ be aa set containing all the statements in a program. $\pi \in ST$ is called an Error Trace iff there is an assertion condition $\phi$, which is violated if we execute $\pi$, starting from a state which belongs to a set of states called Error Precondition $EP$. We call the violation of $\pi$ an error.\\
...
assume( y==3 )
\end{lstlisting}
If we replace $y:=3$ by $hacov(y)$, then $y$ can non-deterministically be assigned any value and we can't be certain any more that the assertion condition in the end $\phi = (y!=3)$ will be violated and we can end up in an error state.
