...
For a set of variables $X$ we denote by $Expr(X)$ the set of terms built from variables in $X$ and we denote by $Preds(X)$ the set of all quantifier- free formulas with free variables in $X$.\\
let $X$ be a fixed set of variables which we call "program variables", and let $L$ be a set of labels.\\
A \textit{program statement} $st$ is either a conditional choice, a while loop, a sequence of statements, an assignment or a label.
$$st := if\ \mathbf{cond}\ then\ \mathbf{st}\ else\ \mathbf{st}\ |\ while\ \mathbf{cond}\ do\ \mathbf{st}\ |\ st;st\ |\ x:=e\ |\ label\ l$$
Labels have no operational meaning. They are only used to uniquely identify certain points during the execution of the program statement.\\
A program $P=<st,\Phi>$ consists of a program statement $st$, and an assertion map.
$$\Phi: L \rightarrow Preds(X)$$
which maps each label $L$ in st to a formula that should hold at the point of execution of $st$ determined by $L$.\\
...
$$st^a  := if\ \mathbf{cond}\ |\ endif\ |\ x:=e\ |\ label\ L\ |\ havoc\ \mathbf{cond}$$
A trace $\pi$ is a finite sequence of atomic statements. Let $\pi$ be a trace of length $n$.\\
A program state $s$ is a function that assigns a value $S(x)$ to each program variable $x \in X$. We call formulas $Preds(X)$ state formulas and we write $s \Rightarrow F$ to denote that a state $s$ satisfies a state formula $F$.\\
The formulas $Preds(X \cup X')$ are called transition formulas. We use transition formulas to represent the semantics of statements in a trace, where the variables $X'$ denote the values of the variables from $X$ in the next state. We write $s,s' \Rightarrow T$ to denote that states $s$ and $s'$ satisfy the transition formula $T$.\\
An execution of a trace $\pi$ of length $n$ is a sequence of states $s_0....s_n$ such that for all $0 \leq i \leq n$, $s_i,s_{i+1} \Rightarrow T[\pi[i]]$. We denote by $Execs(\pi)$ the set of all executions of $\pi$. The trace formula of $\pi$ is the formula:
...
A program $P = <st,\Phi>$ is safe if for every trace $\pi \in Trace(P)$ whose final statement is a label statement and every execution $\sigma$ of $\pi$, the final state of $\sigma$ satisfies $\Phi(l)$.\\
...
\textbf{Error: }For a program $P=<st, \Phi>$ , with a trace $\pi = st_0^a;.....st_{n-2}^a;label\ l$, an error precondition $\Psi$, a state formula $\phi$, such that $\Phi(l)=\phi$, we say that there is an error in the program $P$ if the following conditions hold:\\
1) $\Psi \wedge TF(\pi)$ is satisfiable\\
2) $\Psi \wedge TF(\pi) \wedge \phi^{<n>} $ is unsatisfiable\\
In this case $\pi$ is called an error trace. Hence for an error trace $\pi$, no execution of the trace $\pi$ that starts in a state satisfying the error pre-condition $\Psi$ ends in a state satisfying the postcondition $\phi$.
