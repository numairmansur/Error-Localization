...
\textbf{Error \& Error Trace: }For an unsafe program $P$ , with a trace $\pi = st_0;.....st_{n-1};$ whose corresponding run's last state is an error state, an error precondition $\Psi$, a state formula $\phi$, such that $TF[\pi[n]]=\phi$, we say that there is an \textbf{\textit{error}} in the program $P$ if the following conditions hold :\\
1) $\Psi \wedge TF(\pi)$ is satisfiable.\\
2) $\Psi \wedge TF(\pi) \wedge \phi^{<n>} $ is un-satisfiable.\\
In this case $\pi$ is called an \textbf{\textit{error trace}}. Hence for an error trace $\pi$, no execution of the trace $\pi$ that starts in a state satisfying the \textbf{\textit{error pre-condition}} $\Psi$ ends in an exit state.\\
