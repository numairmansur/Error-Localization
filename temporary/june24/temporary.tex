...
\\ \textbf{(anything more ?)}
\section{Discarded Definitions:}
This section mentions the defintions for relevancy that are pre-versions(?) of the definition that is in the final POPL submission. These defintions have been discarded due to their inability to correctly and completely explain our notion of relevancy. They are in the document primarily for the reason of documenting failed/incorrect versions of our definition that might be useful in the future as we further develop our understanding of what it means that a statement is relevant in an error trace for a simple bug. 
\subsection{The restricitve-assume check:}
I think here we have to talk assignemtn and havoc relevancy seperately. and maybe also talk about the proofs.
...
\subsection{SP Intersection check(Implementational detail)}
bla bla\\
