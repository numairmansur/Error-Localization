...
$$P \not \limp WP(Q,havoc(x))$$
Let $Q' := SP(P; havoc(x))$ and $P' := WP(Q;havoc(x))$. \\
\\
\includegraphics[width=\linewidth]{traces.jpg}\\
Relevncy of $x:=t$ implies that replacing it with $havoc(x)$ gives us more blocking executions then before. Therefore
$$Q \subsetneq Q'$$
The relevancy of the assignment statement $x:=t$ implies that the trace $\pi'$ have strictly more blocking exections then $\pi$. This means that there exists an assume statement in the trace $\pi$ at position $j$, which is blocking more executions then before. Or we can say that there are more states $s_j$ for which the assume statement is blocking. \\
