...
\end{equation}
The above implication does not hold if following 2 conditions holds:\\
1) assume that the left hand side hold then derive that the right hand side does not hold.\\
2) left hand side is not FALSE. That means there is a case where it actually holds.\\
1) Suppose in (2), $\neg WP(\neg guard, assignment)$ holds.\\
then from lemma (3) $\neg WP(\neg guard, havoc)$ holds.\\
and consequently $WP(\neg guard,havoc)$ does not hold.\\
2) P which is the error precondition in this case can not be false because we assumed in the begining that we have an error trace and that we are reaching the error state. \\
