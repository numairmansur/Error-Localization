...
This section mentions two examples with a counterintuitive results when analyzed by our algorithm. For future reference, let us name these examples as follows:
...
One weakness of the error invariants approach \cite{errorinvariants} is it's inability to deal with havoc statements. The other weakeness is related to the fact that generating error invariants require an unsatisfiable trace formula. This leads to a limitiation in the case when there is an assert false at the end of that unsatisfiable trace formula. In this case, all the interpolants can be chosen as true and none of the statements would be considered relevant. We call this the \textit{assert-false problem}. Following is a very example where this problem becomes easy to see:
\begin{lstlisting}
foo()
{ 
	x := 1;
