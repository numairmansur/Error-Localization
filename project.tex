...
Formally: \\
$\pi[i]$ is relevant if $\exists$ a trace $\pi'$, where\\
$\pi'[i]$ is a havoc statement in terms of the variable on the left side of $\pi[i]$ and $\exists$ some assume statement at position $j$ such that \\
$$post(\pi'[0,j-1], pre) = false$$ 
