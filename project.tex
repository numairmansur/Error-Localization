...
2) left hand side is TRUE. That means there is a case where it actually holds.\\
Suppose in (2), $\neg WP(\neg guard, assignment)$ holds.\\
then from lemma (3) $\neg WP(\neg guard, havoc)$ holds.\\
and consequently $WP(\neg guard,havoc)$ does not hold.\\
If in the error trace, the assignment statement is relevant and is of the form such that the formula $x:=t \Rightarrow guard$\\
Then from lemma (4):\\
$WP(guard; x:=t)$ holds $True$\\
$\neg WP(\neg guard; x:=t)$ holds $True$
